...
**Verificação de atualização parece travada / não dispara** — a checagem roda uma vez por sessão a partir do `SessionStart` (`scripts/hooks/session-start.sh`), não a cada tool call. Verifique se `.dev-team-agents/user-data/state.json` é um arquivo gravável (não um diretório) e se `session-start.sh` é executável — a chave `last_update_check` agora vive lá. Os hooks de notifier, telemetria e refresh automático do Graphify estão desativados por padrão no momento — veja `CLAUDE-md/hooks.md` § Disabled Hooks para status e como reativar.
