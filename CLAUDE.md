...
**Codex prompt limitation.** The project-local Codex path is now `.codex/skills/devteam-*/SKILL.md` → `$devteam-*`. Optional slash-command aliases live in `~/.codex/prompts/devteam-*.md`, and those prompt files are plain Markdown prompts, not custom-agent TOML layers, so they do **not** carry runtime `model` or `model_reasoning_effort` fields. In the Codex port, model/effort pinning is enforced by the spawned custom agents in `.codex/agents/*.toml`; prompt-file metadata is documentation only unless Codex introduces prompt-level config later.
