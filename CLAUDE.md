...
`/devteam:update`, `/devteam:symlinks` and `/devteam:health-check` spawn **no** agent; their `agent` field is filler the renderer still requires. All three name `technical-writer`, which keeps the filler consistent and resolves the rule above to `repetitive` — what that class of work actually is. Do not read those three rows as a delegation target.
