...
**Bodies are emitted almost verbatim.** A short "Tool conventions" preamble added per provider explains how each Claude Code tool name the body references maps to the target provider's native tool (e.g., `Task` → `task` in opencode, `Task` → `spawn_agent` in codex, `AskUserQuestion` → `request_user_input` in Codex Plan mode). The Codex renderer also applies a narrow rewrite pass for Claude-specific tool phrasing, converts quiz payload examples into the `request_user_input` shape expected by Codex while preserving the original options, and now chooses the fallback policy per command: operational commands continue by asking the same question directly in conversation, while plan-first commands keep the `/plan` retry requirement.
