...
**Bodies are emitted almost verbatim.** A short "Tool conventions" preamble added per provider explains how each Claude Code tool name the body references maps to the target provider's native tool (e.g., `Task` → `task` in opencode, `Task` → `spawn_agent` in codex, `AskUserQuestion` → `request_user_input` in Codex Plan mode, with plain-text fallback only when that tool is unavailable). The Codex renderer also applies a narrow rewrite pass for Claude-specific tool phrasing and converts quiz payload examples into the `request_user_input` shape expected by Codex while preserving the original options.
