...
- `flow-rollback-no-pre-state-validation-vs-update-sh` — **pendente** — Evidência: `scripts/rollback.sh` (65 linhas) não compara `$TARGET` com `.installed-version` atual (no-op silencioso possível) nem detecta modificações locais não-commitadas em `.claude/dev-team-agents/`.
