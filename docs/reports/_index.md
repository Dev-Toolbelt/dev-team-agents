...
- `docs-sync-extracted-docs-not-translated-to-pt-br` — `docs/agents.md` (6714 B) e `docs/installation.md` (5723 B) criados pelo commit `7800516` apenas em inglês; README.pt-BR.md linka para os mesmos paths; viola "README Sync Rule" — [relatório](2026-05-12/01-referencias-e-consistencia.md)
- `ref-jira-skill-over-500-line-hard-limit` — **HIGH** — `skills/integrations/jira/SKILL.md` tem 516 linhas; CLAUDE.md:139 declara limite ~500; `references/` não existe — [relatório](2026-05-12/01-referencias-e-consistencia.md)
- `ref-large-skills-no-references-subdir-pattern-not-adopted` — 5 skills 300-516 linhas sem `references/` extraction (jira, sonarqube, sentry, docs-sync, cloudflare, iac-terraform); padrão é exceção, não regra — [relatório](2026-05-12/01-referencias-e-consistencia.md)
- `ref-design-skills-non-standard-frontmatter-keys` — `web-design-guidelines/SKILL.md` tem bloco `metadata:` (author/version/argument-hint); `frontend-design/SKILL.md` tem `license:`; CLAUDE.md aceita só `name`+`description` — [relatório](2026-05-12/01-referencias-e-consistencia.md)
...
- `ref-ci-yml-no-skill-size-limit-validation` — CI valida frontmatter/shellcheck/README-sync mas não tamanho de agent/skill; as 5 violações ativas passaram em PRs anteriores sem alerta — [relatório](2026-05-12/01-referencias-e-consistencia.md)
- `ref-claude-md-file-structure-misses-new-docs-children` — bloco File Structure em CLAUDE.md:182-203 lista `docs/` sem subdivisão; após `7800516`, `docs/agents.md` e `docs/installation.md` ficam invisíveis na tree declarativa — [relatório](2026-05-12/01-referencias-e-consistencia.md)
- `ref-changelog-package-exclusions-update-not-documented-in-unreleased` — commit `09c00ca` expandiu tabela "Package exclusions" mas não registrou no `[Unreleased]` do CHANGELOG; contradiz auto-docs rule — [relatório](2026-05-12/01-referencias-e-consistencia.md)
- `ref-claude-md-immutability-contract-vs-actual-symlinks-rule-drift` — CLAUDE.md:213-219 afirma "files installed via symlinks"; install.sh usa `mv` em path fixo — descrição contradiz implementação — [relatório](2026-05-12/01-referencias-e-consistencia.md)
- `ref-orphan-scan-covers-agents-only-but-skills-may-be-loaded-only-by-commands` — variante de fingerprint pendente: hoje `current-context` (23 commands) e `spawn-classifier` (1 command) são command-level; manutenção é via allowlist na tabela CLAUDE.md (falso positivo iminente) — [relatório](2026-05-12/01-referencias-e-consistencia.md)
- `ref-package-exclusions-table-mechanism-column-overlap-with-install-sh-comments` — tabela CLAUDE.md tem coluna "Mechanism", `install.sh` não tem comentários inline mapeando `rm -f` à justificativa — [relatório](2026-05-12/01-referencias-e-consistencia.md)

#### Fluxos e Workflows (10)

- `flow-plan-command-does-not-load-plan-mode-skill` — **HIGH** — `commands/plan.md` não carrega `skills/shared/plan-mode/SKILL.md` apesar de ser o command de geração de plano; 7 agents spawnados carregam, command parent não — [relatório](2026-05-12/02-fluxos-e-workflows.md)
- `flow-installer-hook-coverage-still-3-of-7-after-session-start-added` — 4 hook events ainda sem registro (UserPromptSubmit, SubagentStop, Notification, PreCompact); recomendar PreCompact primeiro para garantir session-summary pre-compact — [relatório](2026-05-12/02-fluxos-e-workflows.md)
- `flow-review-command-no-explicit-plan-gate-undocumented-exception` — `commands/review.md` é único sem PLAN GATE entre 21 commands; exceção tácita por ser read-only mas não documentada — [relatório](2026-05-12/02-fluxos-e-workflows.md)
- `flow-stop-dispatcher-runs-all-4-sub-scripts-without-fast-path` — `stop.sh` executa 4 sub-scripts sem flag `DEVTEAM_NO_CHANGES=1`; overhead ~200ms/Stop × 30 turns = ~6s/sessão em read-only — [relatório](2026-05-12/02-fluxos-e-workflows.md)
- `flow-no-pre-spawn-current-context-warm-cache` — 23 commands carregam current-context antes do spawn; cada agent re-executa `git diff main..HEAD` (~5-10s × agents); falta cache curto em `.claude/user-data/.context-cache.json` — [relatório](2026-05-12/02-fluxos-e-workflows.md)
- `flow-spawn-classifier-only-plan-but-fix-refactor-fullstack-multi-agent-too` — sub-escopo: 7 commands declaram spawn condicional em CLAUDE.md (backend, frontend, fullstack, fix, refactor, review, mobile); apenas plan.md carrega spawn-classifier; outros usam heurística inline — [relatório](2026-05-12/02-fluxos-e-workflows.md)
- `flow-no-workflow-mobile-md-and-workflow-design-md-files` — sub-escopo: `commands/mobile.md` e `commands/design.md` existem mas sem `workflows/mobile.md` / `workflows/design.md`; assimetria com fullstack/backend/frontend — [relatório](2026-05-12/02-fluxos-e-workflows.md)
- `flow-pr-command-no-conventional-commits-validation-of-staged-commits` — `commands/pr.md` não valida se commits da branch seguem Conventional Commits; PR pode ser aberta com história inconsistente — [relatório](2026-05-12/02-fluxos-e-workflows.md)
- `flow-update-command-no-pre-update-backup-strategy-after-revert-fc57a86` — sub-escopo (vs ↩️ Reverted): alternativas leves a `.previous/` não exploradas (git tag pre-update, `scripts/rollback.sh` re-baixando por tag); update.sh hoje é zero-backup — [relatório](2026-05-12/02-fluxos-e-workflows.md)
- `flow-refactor-workflow-no-tag-checkpoint-still-after-checkpoints-added` — sub-escopo: refactor.md tem 5 CHECKPOINTs semânticos mas zero git-tag físico (`pre-refactor-<scope>`); rollback exige `git reflog` manual — [relatório](2026-05-12/02-fluxos-e-workflows.md)

#### Agentes e Skills (12)

- `agent-software-architect-foundational-rule-51-lines-2x-avg` — Foundational Rule de software-architect é 51 linhas (p50=22, +132%); outlier; spawned em 9 commands ⇒ ~1.900 tokens/dia — [relatório](2026-05-12/03-agentes-e-skills.md)
...
- `skill-token-efficiency-not-loaded-by-six-non-coding-agents` — `product-analyst`, `qa-specialist`, `security-specialist`, `setup-assistant`, `technical-writer`, `ui-ux-designer` não carregam `token-efficiency`; agents de discovery especialmente são "Read-heavy" — [relatório](2026-05-12/03-agentes-e-skills.md)
- `agent-mobile-developer-no-detox-or-maestro-test-routing-still-pending` — reforço: zero hits para detox/maestro/appium em mobile-developer; opção A (criar mobile-test-specialist) vs opção B (seção routing inline) — [relatório](2026-05-12/03-agentes-e-skills.md)
...
- `agent-tools-frontmatter-canonical-order-not-enforced` — `agent-lint.sh` valida presença mas não ordem canônica de tools; sem regra "Read sempre primeiro"; novos agents podem importar qualquer ordem — [relatório](2026-05-12/03-agentes-e-skills.md)
- `skill-monitoring-references-folder-exists-but-empty` — pasta `monitoring/references/` criada mas vazia; sinal contraditório (intenção registrada, ação não tomada); decidir se popular ou remover — [relatório](2026-05-12/03-agentes-e-skills.md)
- `agent-product-analyst-loads-jira-skill-but-not-other-trackers` — variante: jira carregada por product-analyst; Linear/Asana skills (existem ou não) sem load condicional; tracker detection ausente — [relatório](2026-05-12/03-agentes-e-skills.md)
- `skill-frontmatter-strict-validation-missing-from-lint` — `agent-lint.sh` valida agents mas não SKILL.md; as 2 violações em design skills (3 chaves) não foram pegas por CI — [relatório](2026-05-12/03-agentes-e-skills.md)
- `skill-discovery-mode-three-agents-need-explicit-collision-protocol` — sub-escopo: `discovery-mode` carregada por 3 agents; sem lockfile `.claude/.discovery-lock.json`; outputs divergem em `/devteam:plan` — [relatório](2026-05-12/03-agentes-e-skills.md)
...
- `skill-reviewer-base-loaded-after-project-context-but-overlaps-7-steps` — sub-escopo: análise linha a linha — 5 dos 11 steps em reviewer-base duplicam project-context; potencial reduzir reviewer-base de 11 → 6 steps — [relatório](2026-05-12/03-agentes-e-skills.md)

#### Economia de Tokens (10)

- `token-foundational-rule-software-architect-outlier-51-lines` — sub-escopo (vs 424-linhas global): poda cirúrgica em software-architect (51→25); ~210 tokens × 9 contextos = ~1.900 tokens/dia — [relatório](2026-05-12/04-economia-tokens.md)
...
- `token-skill-monitoring-444-lines-loaded-by-devops-and-architect` — quantificação: 444 linhas × 2 contextos = ~7.100 tokens; pós-extração para references/ → ~1.300 tokens; **economia ~5.800/sessão** — [relatório](2026-05-12/04-economia-tokens.md)
- `token-readme-228-each-pos-extraction-but-ci-sync-still-line-based` — variante: README hoje 228 linhas cada (-69% vs 734); CI ainda usa threshold 5% line-count (12 linhas absolutas); insuficiente para drift estrutural — [relatório](2026-05-12/04-economia-tokens.md)
- `token-plan-mode-143-lines-7-agents-but-only-1-command-loads` — ironia: `/devteam:plan` (caso primário) não carrega; 7 agents carregam eagerly; mover load para command e remover dos agents = ~7.200 tokens/sessão — [relatório](2026-05-12/04-economia-tokens.md)
...
- `token-stop-hook-04-sub-scripts-200ms-overhead-on-read-only-sessions` — quantificação: ~200ms/Stop × 30 turns = 6s/sessão; passar flag `DEVTEAM_NO_CHANGES` reduz a ~40ms (-73%) em sessões read-only — [relatório](2026-05-12/04-economia-tokens.md)
...
- `token-orphan-scan-script-shells-13-times-per-execution` — `orphan-skill-scan.sh` faz ~13 finds + ~50 greps por execução; batch single-find + xargs/awk reduz a 2 calls — [relatório](2026-05-12/04-economia-tokens.md)
- `token-comments-policy-417-lines-still-monolith-no-section-loading` — variante mecanismo: propor padrão `sections/<lang>.md` reaproveitável; comments-policy 417 → ~120 linhas/agent × 9 agents = **~18.700 tokens/sessão** (maior ganho do audit) — [relatório](2026-05-12/04-economia-tokens.md)
...
| 2026-05-11 | 50 + auditoria Guardian | 194 | **13** ✅ + 3 ⚠️ partial + 1 🟢 resolved + 33 pendentes (verificado por Guardian em 2026-05-12) |
| 2026-05-12 | 44 + auditoria Guardian | 238 | — |
