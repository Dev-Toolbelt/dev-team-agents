...
| `ref-claude-md-immutability-contract-vs-actual-symlinks-rule-drift` | ✅ **Executed** | `grep "fixed path" CLAUDE.md` confirma: "These files are installed at a fixed path (`.claude/dev-team-agents/`) and replaced entirely on every update." Sem menção a "symlinks" — descrição alinhada com `mv` real do install.sh |
