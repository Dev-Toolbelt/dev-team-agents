...
| `/devteam:health-check` | Installation diagnostics — detects the active provider (Claude / opencode / Codex), runs 13 checks (symlinks, scripts, user data, provider config, graphify, CLAUDE.md, .gitignore, preferences, notifier, credentials, memory artifacts, python prerequisite, productivity/token-efficiency tools) and applies safe auto-fixes. Almost never deletes: anything it can't place is moved to `.dev-team-agents/user-data/legacy/<date>/`, files that hold accumulated knowledge are adapted in place never regenerated, and the sole exception is a confirmed `.pre-migration.bak` whose value already lives in `state.json` |
...
| `/devteam:version` | Version check — prints the installed dev-team-agents version in the session-start banner layout; one bash call, no agent spawn, minimum token cost |
| `/devteam:status` | Git status snapshot — branch/worktree, unstaged and staged changes, last 5 commits, and totals as formatted markdown tables; one bash call, no agent spawn, minimum token cost. `/devteam:status <branch-name>` inspects that branch instead of the current one |
...
**Update check seems stuck / not firing** — the check runs once per session from `SessionStart` (`scripts/hooks/session-start.sh`), not on every tool call. Verify `.dev-team-agents/user-data/state.json` is a writable file (not a directory) and that `session-start.sh` is executable — the `last_update_check` key lives there now. The notifier, telemetry, and automatic Graphify refresh hooks are currently disabled by default — see `CLAUDE-md/hooks.md` § Disabled Hooks for status and how to re-enable.
