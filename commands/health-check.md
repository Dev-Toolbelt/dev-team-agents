...
- **codex**: check `.codex/hooks.json` — the 4 managed hook entries (SessionStart, PreToolUse, Stop, PreCompact) are present and point to valid script paths; check `prompts/` dir for `devteam-*.md` files; check `.codex/skills/` for generated `devteam-*` skill dirs with `SKILL.md`; verify agent model/effort mapping in `.codex/agents/*.toml` against `.dev-team-agents/scripts/lib/tiers.json` (including `agent_effort` overrides)
