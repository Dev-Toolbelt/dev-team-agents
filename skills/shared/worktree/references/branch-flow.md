...
from `.dev-team-agents/user-data/preferences.json` (default `.worktrees`). The full
...
```bash
python3 -c "import json;d=json.load(open('.dev-team-agents/user-data/preferences.json'));print(d.get('worktree_base_branch') or '', d.get('worktree_path','.worktrees'), d.get('worktree_docker_isolate',True))" 2>/dev/null
```
