...
| **Integration points** | Data flows correctly between services and modules; API contracts honored (shapes match spec); auth flows with valid token, expired token, no token, wrong role; third-party integrations behave as expected. Incidental only: a "unit" test hitting a real DB/API/filesystem/storage → `[cross-boundary → test-specialist]` per the Hard rule in `skills/testing/test-pyramid/SKILL.md` |
